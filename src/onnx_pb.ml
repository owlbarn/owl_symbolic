...
let rec decode_type_proto_value d = 
  let rec loop () = 
    let ret:Onnx_types.type_proto_value = match Pbrt.Decoder.key d with
      ...
      | Some (1, _) -> Onnx_types.Tensor_type (decode_type_proto_tensor (Pbrt.Decoder.nested d))
      | Some (4, _) -> Onnx_types.Sequence_type (decode_type_proto_sequence (Pbrt.Decoder.nested d))
      | Some (5, _) -> Onnx_types.Map_type (decode_type_proto_map (Pbrt.Decoder.nested d))
      | Some (n, payload_kind) -> (
        Pbrt.Decoder.skip d payload_kind; 
        loop () 
      )
