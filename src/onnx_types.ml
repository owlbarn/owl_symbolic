...
let rec default_sparse_tensor_proto 
  ?values:((values:tensor_proto option) = None)
  ?indices:((indices:tensor_proto option) = None)
  ?dims:((dims:int64 list) = [])
  () : sparse_tensor_proto  = {
  values;
  indices;
...
let rec default_tensor_shape_proto_dimension_value () : tensor_shape_proto_dimension_value = Dim_value (0L)
...
let rec default_tensor_shape_proto 
  ?dim:((dim:tensor_shape_proto_dimension list) = [])
  () : tensor_shape_proto  = {
  dim;
}
...
let rec default_type_proto_value () : type_proto_value = Tensor_type (default_type_proto_tensor ())
...
and default_type_proto_sequence 
  ?elem_type:((elem_type:type_proto option) = None)
  () : type_proto_sequence  = {
  elem_type;
}
...
let rec default_attribute_proto 
  ...
  ?floats:((floats:float list) = [])
  ?ints:((ints:int64 list) = [])
  ?strings:((strings:bytes list) = [])
  ?tensors:((tensors:tensor_proto list) = [])
  ?graphs:((graphs:graph_proto list) = [])
  ?sparse_tensors:((sparse_tensors:sparse_tensor_proto list) = [])
  () : attribute_proto  = {
  name;
  ref_attr_name;
  doc_string;
...
and default_graph_proto 
  ...
  ?quantization_annotation:((quantization_annotation:tensor_annotation list) = [])
  () : graph_proto  = {
  node;
  name;
  initializer_;
