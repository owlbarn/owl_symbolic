...
let default_tensor_shape_proto_dimension_mutable ()
    : tensor_shape_proto_dimension_mutable
  =
  { value = Onnx_types.Dim_value 0L; denotation = "" }
...
let rec decode_tensor_proto_segment d =
  ...
  while !continue__ do
    ...
    | None ->
      ();
      continue__ := false
    | Some (1, Pbrt.Varint) -> v.begin_ <- Pbrt.Decoder.int64_as_varint d
    | Some (1, pk) ->
      Pbrt.Decoder.unexpected_payload "Message(tensor_proto_segment), field(1)" pk
    | Some (2, Pbrt.Varint) -> v.end_ <- Pbrt.Decoder.int64_as_varint d
    | Some (2, pk) ->
...
let rec decode_tensor_proto d =
  ...
  while !continue__ do
    ...
    | Some (2, Pbrt.Varint) -> v.data_type <- Pbrt.Decoder.int32_as_varint d
    | Some (2, pk) ->
    ...
    | Some (8, Pbrt.Bytes) -> v.name <- Pbrt.Decoder.string d
    | Some (8, pk) ->
      Pbrt.Decoder.unexpected_payload "Message(tensor_proto), field(8)" pk
    | Some (12, Pbrt.Bytes) -> v.doc_string <- Pbrt.Decoder.string d
    | Some (12, pk) ->
      Pbrt.Decoder.unexpected_payload "Message(tensor_proto), field(12)" pk
    | Some (9, Pbrt.Bytes) -> v.raw_data <- Pbrt.Decoder.bytes d
    | Some (9, pk) ->
    ...
    | Some (14, Pbrt.Varint) -> v.data_location <- decode_tensor_proto_data_location d
    | Some (14, pk) ->
...
let rec decode_tensor_shape_proto_dimension_value d =
  let rec loop () =
    let ret : Onnx_types.tensor_shape_proto_dimension_value =
      ...
      | Some (1, _)            -> Onnx_types.Dim_value (Pbrt.Decoder.int64_as_varint d)
      | Some (2, _)            -> Onnx_types.Dim_param (Pbrt.Decoder.string d)
      | Some (n, payload_kind) ->
  ...
  while !continue__ do
    ...
    | Some (1, pk) ->
      Pbrt.Decoder.unexpected_payload
        "Message(tensor_shape_proto_dimension), field(1)"
        pk
    | Some (2, Pbrt.Bytes) -> v.value <- Onnx_types.Dim_param (Pbrt.Decoder.string d)
    | Some (2, pk) ->
      Pbrt.Decoder.unexpected_payload
        "Message(tensor_shape_proto_dimension), field(2)"
        pk
    | Some (3, Pbrt.Bytes) -> v.denotation <- Pbrt.Decoder.string d
    | Some (3, pk) ->
      Pbrt.Decoder.unexpected_payload
        "Message(tensor_shape_proto_dimension), field(3)"
        pk
    | Some (_, payload_kind) -> Pbrt.Decoder.skip d payload_kind
  done;
...
let rec decode_type_proto_tensor d =
  ...
  while !continue__ do
    ...
    | None ->
      ();
      continue__ := false
    | Some (1, Pbrt.Varint) -> v.elem_type <- Pbrt.Decoder.int32_as_varint d
    | Some (1, pk) ->
...
and decode_type_proto d =
  ...
  while !continue__ do
    ...
    | Some (1, Pbrt.Bytes) ->
      v.value
        <- Onnx_types.Tensor_type (decode_type_proto_tensor (Pbrt.Decoder.nested d))
    | Some (1, pk) -> Pbrt.Decoder.unexpected_payload "Message(type_proto), field(1)" pk
    | Some (4, Pbrt.Bytes) ->
      v.value
        <- Onnx_types.Sequence_type (decode_type_proto_sequence (Pbrt.Decoder.nested d))
    | Some (4, pk) -> Pbrt.Decoder.unexpected_payload "Message(type_proto), field(4)" pk
    | Some (5, Pbrt.Bytes) ->
      v.value <- Onnx_types.Map_type (decode_type_proto_map (Pbrt.Decoder.nested d))
    | Some (5, pk) -> Pbrt.Decoder.unexpected_payload "Message(type_proto), field(5)" pk
    | Some (6, Pbrt.Bytes) -> v.denotation <- Pbrt.Decoder.string d
    | Some (6, pk) -> Pbrt.Decoder.unexpected_payload "Message(type_proto), field(6)" pk
    | Some (_, payload_kind) -> Pbrt.Decoder.skip d payload_kind
  done;
  ({ Onnx_types.value = v.value; Onnx_types.denotation = v.denotation }
...
and decode_type_proto_map d =
  ...
  while !continue__ do
    ...
    | None ->
      ();
      continue__ := false
    | Some (1, Pbrt.Varint) -> v.key_type <- Pbrt.Decoder.int32_as_varint d
    | Some (1, pk) ->
...
let rec decode_value_info_proto d =
  ...
  while !continue__ do
    ...
    | Some (3, Pbrt.Bytes) -> v.doc_string <- Pbrt.Decoder.string d
    | Some (3, pk) ->
...
let rec decode_tensor_annotation d =
  ...
  while !continue__ do
    ...
    | None ->
      ...
      continue__ := false
    | Some (1, Pbrt.Bytes) -> v.tensor_name <- Pbrt.Decoder.string d
    | Some (1, pk) ->
...
and decode_graph_proto d =
  ...
  while !continue__ do
    ...
    | Some (1, pk) -> Pbrt.Decoder.unexpected_payload "Message(graph_proto), field(1)" pk
    | Some (2, Pbrt.Bytes) -> v.name <- Pbrt.Decoder.string d
    | Some (2, pk) -> Pbrt.Decoder.unexpected_payload "Message(graph_proto), field(2)" pk
    | Some (5, Pbrt.Bytes) ->
      v.initializer_ <- decode_tensor_proto (Pbrt.Decoder.nested d) :: v.initializer_
    | Some (5, pk) -> Pbrt.Decoder.unexpected_payload "Message(graph_proto), field(5)" pk
    | Some (15, Pbrt.Bytes) ->
    ...
    | Some (10, Pbrt.Bytes) -> v.doc_string <- Pbrt.Decoder.string d
    | Some (10, pk) ->
...
and decode_node_proto d =
  ...
  while !continue__ do
    ...
    | Some (1, pk) -> Pbrt.Decoder.unexpected_payload "Message(node_proto), field(1)" pk
    | Some (2, Pbrt.Bytes) -> v.output <- Pbrt.Decoder.string d :: v.output
    | Some (2, pk) -> Pbrt.Decoder.unexpected_payload "Message(node_proto), field(2)" pk
    | Some (3, Pbrt.Bytes) -> v.name <- Pbrt.Decoder.string d
    | Some (3, pk) -> Pbrt.Decoder.unexpected_payload "Message(node_proto), field(3)" pk
    | Some (4, Pbrt.Bytes) -> v.op_type <- Pbrt.Decoder.string d
    | Some (4, pk) -> Pbrt.Decoder.unexpected_payload "Message(node_proto), field(4)" pk
    | Some (7, Pbrt.Bytes) -> v.domain <- Pbrt.Decoder.string d
    | Some (7, pk) -> Pbrt.Decoder.unexpected_payload "Message(node_proto), field(7)" pk
    | Some (5, Pbrt.Bytes) ->
      v.attribute <- decode_attribute_proto (Pbrt.Decoder.nested d) :: v.attribute
    | Some (5, pk) -> Pbrt.Decoder.unexpected_payload "Message(node_proto), field(5)" pk
    | Some (6, Pbrt.Bytes) -> v.doc_string <- Pbrt.Decoder.string d
    | Some (6, pk) -> Pbrt.Decoder.unexpected_payload "Message(node_proto), field(6)" pk
    | Some (_, payload_kind) -> Pbrt.Decoder.skip d payload_kind
  done;
  ({ Onnx_types.input = v.input
   ; Onnx_types.output = v.output
...
let rec decode_operator_set_id_proto d =
  ...
  while !continue__ do
    ...
    | None ->
      ();
      continue__ := false
    | Some (1, Pbrt.Bytes) -> v.domain <- Pbrt.Decoder.string d
    | Some (1, pk) ->
      Pbrt.Decoder.unexpected_payload "Message(operator_set_id_proto), field(1)" pk
    | Some (2, Pbrt.Varint) -> v.version <- Pbrt.Decoder.int64_as_varint d
    | Some (2, pk) ->
...
let rec decode_model_proto d =
  ...
  while !continue__ do
    ...
    | None ->
      ...
      continue__ := false
    | Some (1, Pbrt.Varint) -> v.ir_version <- Pbrt.Decoder.int64_as_varint d
    | Some (1, pk) -> Pbrt.Decoder.unexpected_payload "Message(model_proto), field(1)" pk
    | Some (8, Pbrt.Bytes) ->
      v.opset_import
        <- decode_operator_set_id_proto (Pbrt.Decoder.nested d) :: v.opset_import
    | Some (8, pk) -> Pbrt.Decoder.unexpected_payload "Message(model_proto), field(8)" pk
    | Some (2, Pbrt.Bytes) -> v.producer_name <- Pbrt.Decoder.string d
    | Some (2, pk) -> Pbrt.Decoder.unexpected_payload "Message(model_proto), field(2)" pk
    | Some (3, Pbrt.Bytes) -> v.producer_version <- Pbrt.Decoder.string d
    | Some (3, pk) -> Pbrt.Decoder.unexpected_payload "Message(model_proto), field(3)" pk
    | Some (4, Pbrt.Bytes) -> v.domain <- Pbrt.Decoder.string d
    | Some (4, pk) -> Pbrt.Decoder.unexpected_payload "Message(model_proto), field(4)" pk
    | Some (5, Pbrt.Varint) -> v.model_version <- Pbrt.Decoder.int64_as_varint d
    | Some (5, pk) -> Pbrt.Decoder.unexpected_payload "Message(model_proto), field(5)" pk
    | Some (6, Pbrt.Bytes) -> v.doc_string <- Pbrt.Decoder.string d
    | Some (6, pk) -> Pbrt.Decoder.unexpected_payload "Message(model_proto), field(6)" pk
    | Some (7, Pbrt.Bytes) ->
      v.graph <- Some (decode_graph_proto (Pbrt.Decoder.nested d))
    | Some (7, pk) -> Pbrt.Decoder.unexpected_payload "Message(model_proto), field(7)" pk
    | Some (14, Pbrt.Bytes) ->
      v.metadata_props
...
let rec encode_tensor_proto (v : Onnx_types.tensor_proto) encoder =
  ...
  Pbrt.Encoder.nested
    (fun encoder ->
      List.iter
        (fun x -> Pbrt.Encoder.float_as_bits64 x encoder)
  ...
  Pbrt.Encoder.nested
    (fun encoder ->
      List.iter
        (fun x -> Pbrt.Encoder.int64_as_varint x encoder)
